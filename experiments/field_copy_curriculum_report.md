...
### 3.23 Hard Value Rejected DPO Data

基于 3.22 的结论，继续构造更难的 rejected。新数据只替换 value：
...
实现时修正了一个细节：不能把 station 名中的数字当作候选 value，例如 `YEBES40M` 中的 `40`、`NYALES20` 中的 `20`。因此数字提取需要避开字母数字内部的数字片段。
...
hard value DPO data:
SFT baseline preference accuracy: 75.00%
avg margin: 3.1764

DPO 100 on hard value data:
preference accuracy: 75.63%
avg margin: 3.2077
```

结论：hard value rejected 成功把偏好任务变难。旧 DPO 100 几乎没有解决这批数据，说明上一轮 DPO 主要学到的是 easy rejected 边界，而不是最核心的 value binding 边界。

这批数据适合作为下一轮 DPO 的训练集。

### 3.24 Hard Value DPO 100

使用 hard value preference 数据训练 100 step：

```text
init checkpoint: out/sft_mixed_binding_multi_hard_2200/ckpt.pt
dpo data: data/dpo/field_hard_value_800.jsonl
learning rate: 1e-5
beta: 0.1
max iters: 100
```

训练日志中，step 0 仍然是正确的 0.693 起点：
...
由于 eval 每次只抽 20 条样本，训练过程中的 val preference accuracy 抖动较大。完整 preference eval 结果如下：
...
这个结果是负结果 / mixed result：

```text
hard value preference accuracy 没有提升
avg margin 略升
hard binding 小幅提升
distractor 小幅提升
multi-station 下滑
...
这说明 harder rejected 的方向是对的，但当前 DPO 超参并没有把 hard value preference 转化为稳定能力提升。更难的数据只是必要条件，不是充分条件。

下一步如果继续 DPO，应该做小型超参对照：
