...
TEMPLATES = [
    "value = {spaced}",
    "measurement value = {spaced}",
    "reported value is {spaced}",
]


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--out", type=str, default="data/sft/value_copy_spaced_500.jsonl")
...
def build_example(value, template):
    spaced = space_value(value)
    inputText = template.format(spaced=spaced)
    outputText = f"value: {spaced}"
